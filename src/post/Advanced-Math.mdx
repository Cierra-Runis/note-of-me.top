---
title: 考研数学
date: '2024-06-25T21:37:04+08:00'
id: TSJDKA
---

请确保已经了解了 [高中数学](/post/High-School-Math) 相关知识
...
\left\{
  \begin{align*}
    &0 & f(x) \text{为奇函数}\\
    &2 \int_{0}^{+\infty} f(x) & f(x) \text{为偶函数}\\
  \end{align*}
\right.
$$
...
I &= \displaystyle\int_{0}^{\frac{\pi}{2}} \sin^m{x} \cos^n{x} \mathrm{d}x\\
  ...
  &= \left\{
       \begin{align*}
          &\dfrac{(m - 1)!!(n - 1)!!}{(m + n)!!} & m,n \text{不均为偶数}\\
          &\dfrac{(m - 1)!!(n - 1)!!}{(m + n)!!} \cdot \frac{\pi}{2} & m,n \text{均为偶数}\\
       \end{align*}
     \right.
\end{align*}
$$
...
$$
\begin{align*}
  \text{二元函数两个偏导都在} (x_0, y_0) \text{连续} \implies& f(x, y) \text{在} (x_0, y_0) \text{可微} \implies f(x, y) \text{在} (x_0, y_0) \text{连续}\\
  &\Downarrow\\
  &\text{二元函数两个偏导都存在}
\end{align*}
$$
