...
使用什么快捷键来呼出快捷栏呢？在 Windows 上我是 <Kbd keys={['ctrl', 'win', 'alt', 'space']}/>，在 macOS 上 ……

我哪知道，macOS 的键位好奇怪，我键盘切换到 macOS 模式后把 <Kbd keys={['win']}/> 和 <Kbd keys={['alt']}/> 互调了，然后我又在 macOS 设置里将 <Kbd keys={['command']}/> 和 <Kbd keys={['ctrl']}/> 对调了，这样才使得 macOS 的键位和 Windows 一致。
...
如：Windows 上 <Kbd keys={['win']}>V</Kbd> 的剪切板，QQ 提供的 <Kbd keys={['ctrl', 'alt']}>A</Kbd> 截图功能，Windows 上的 Everything 全局搜索功能。
