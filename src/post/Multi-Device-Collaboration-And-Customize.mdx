---
title: '多设备协同与自定义'
date: '2024-03-08T08:44:23+08:00'
---
...
|    平板    |  [iPad Pro 2021 11 inch]  |                               单雷雳 4 40 Gb/s 可 DP 1.4；蓝牙 5.0                               |
|  蓝牙耳机  |     [AirPods Pro II]      |                                        单 USB-C；蓝牙 5.3                                        |
|  ~~Mac~~   |     ~~[Mac mini M2]~~     |                 ~~双雷雳 4 40 Gb/s 可 DP 1.4；双 USB-A；单 HDMI 2.0；蓝牙 5.3~~                  |
|    Mac     |       [Mac mini M4]       |          正双 USB 3 10 Gb/s USB-C；反三雷雳 4 40 Gb/s 可 DP 1.4；单 HDMI 2.1；蓝牙 5.3           |
|   显示器   | 泰坦军团 P2SA2V 4K 144 Hz | 单 DP 1.4 双 HDMI 2.1 均 DSC；单 USB-C 可 DP 1.2、PD 65 W，单 USB-B，其一作双 USB 3.0 USB-A 上行 |
| 笔记本电脑 |   [拯救者 R7000P 2021]    |  单 HDMI 2.1；双 USB 3 10 Gb/s USB-C 可 DP 1.4，其一 PD 100 W；四 USB 3 5 Gb/s USB-A；蓝牙 5.1   |
|    键盘    |  狼蛛客制化旗舰 F99 系列  |   单 USB-C，回报率 1000 Hz；蓝牙 5.0，回报率 125 Hz；无线 2.4 G，回报率 1000 Hz；电池 8000 mAh   |
...
| :------------: | :----------: | :-----------------: | :---------: | :--------: | :--------------------: | :--------------------: |
|      带宽      |   10 Gb/s    |       40 Gb/s       |   80 Gb/s   | 20/40 Gb/s |        40 Gb/s         |        80 Gb/s         |
|      PCIe      |      -       |          -          |      -      |  16 Gb/s   |        32 Gb/s         |        64 Gb/s         |
...
|    视频传输    |  4K 120 Hz   | 4K 240 Hz、8K 60 Hz |    双 4K    |   双 4K    |         双 4K          |         双 8K          |
...
首先将笔记本电脑和显示器用 USB-C 转 [DP](https://en.wikipedia.org/wiki/DisplayPort) 的线连接起来，显示器能以 4K 144 Hz 的预期模式显示画面，接着将鼠标和键盘 2.4 G 接收器插至显示器 USB-A 口。

问题出现了，照理说这两外设应该连上笔记本电脑的，DP 协议是能够运送双向 USB 信号的 —— 总之再从笔记本电脑连出根 USB-C 转 USB-B 的线，显示器设置使 USB-B 作为上行。

再就是将显示器和 Mac 用双公口 USB-C 的线连接起来，显示器能以 4K 60 Hz 的 **惨淡** 模式显示画面 —— 询问客服后得知，显示器的 USB-C 口最高仅支持 4K 60 Hz。
...
在更换 Mac mini M4 版本后，原先的 HDMI 2.0 接口变成了 HDMI 2.1，那么这个接口就能解决我们之前的问题了，在此使用一根海贝思的支持 8K 60 Hz（向下兼容 4K 240 Hz）的双 HDMI 公口线将 Mac mini 连接显示器，显示器能以 4K 144 Hz 的模式显示画面。

是的，如果使用这根线连接笔记本电脑的话，显示器并不能以 4K 144 Hz 的模式显示画面，只能支持到 4K 120 Hz，貌似是 [DSC 显示流压缩技术](https://en.wikipedia.org/wiki/Display_Stream_Compression) 并没起作用 …… 虽然说这也是挺高的刷新率了，但我就是希望顶满 144 Hz，因而笔记本电脑和显示器只能用 USB-C 转 DP 的线连接。
