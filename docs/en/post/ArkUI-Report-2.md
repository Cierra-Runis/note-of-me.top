...
状态管理是抄都抄不懂啊！就连最简单的父组件的给子组件传参，父组件的状态变化了不影响子组件，要多在子组件参数加上 `@Link` 注解，那这样我要是子组件有很多参数依赖是不是得把参数都加上 `@Link` 注解？那如此的话 [模板代码](https://en.wikipedia.org/wiki/Boilerplate_code) 量不就又上去了？
