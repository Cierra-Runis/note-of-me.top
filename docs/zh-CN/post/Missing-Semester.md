...
> [!TIP] 2025-01-11 14:25:09 提醒
>
> Python 的 `os.path` 已经逐渐被推荐用 `pathlib` 替代，更现代化，也更易读

令我印象比较深刻的是 [LeetCode 71 题](https://leetcode.com/problems/simplify-path)，其要求简化一个 Unix 风格的路径，非常推荐大家去做一下。

## Shell 与 Bash

这一节内容实际上在 [SSH](./SSH) 中已经提到了，这里再补充一些。
...
> 你不能强求每个用这个脚本的人都去安装 Python。
>
> 首先是 Windows 已经内置 PowerShell、Linux 和 macOS 内置 bash 了，使用系统原有的东西可以减少一个依赖，当然也有问题就是要准备 `.ps1` 和 `.sh` 两个脚本文件（确实有很多仓库就是怎么做的）。
>
> 再就是 Python 脚本需要安装的 Python 版本，**可能** 会和系统已有的版本冲突，一些依赖也 **可能** 污染已有的环境。当然，使用 uv 喵，使用 uv 谢谢喵。但 uv 也还是要你去下载使用的，就看项目实际能不能，或者愿不愿意推这个工具。
>
> 最后，如果项目本身就是多语言开发，包含有 Python 的，那就无可厚非了。
