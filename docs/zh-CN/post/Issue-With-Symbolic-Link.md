...
在之前的 [云盘同步及 Obsidian 相关](./Cloud-Drive-Sync-And-Obsidian-Related) 文章里有提到：
...
根据以上步骤网盘存储真实数据，使用处使用符号链接将我项目里的 `_post` 文件夹改为指向 iCloud 里的 `帖子` 文件夹后，用 VS Code 打开项目文件夹会看到 `_post` 文件夹后提示这是一个符号链接，和平常一样没有任何问题。
